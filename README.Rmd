...
The goal of r-test-matplotlib is testing that Python `matplotlib` works seemlessly from within `RStudio 1.2-preview`. 

## Motivation
Making `matplotlib` run from within RStudio using the R package `reticulate` and Python Anaconda is a very difficult task. PyQt is not recognized by RStudio, and requires from the user to provide a path to the Qt plugins adding instability to teh R-Python installation.
...
## Examples
I have included some working examples of Rmarkdown notebooks with matplotlib plots.
...
## WinPython
I downloaded WinPython 3.6 64-bit from [here](https://sourceforge.net/projects/winpython/files/WinPython_3.6/3.6.7.0/).
